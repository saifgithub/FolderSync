...
final class DatabaseManager {
    ...
    private func databaseFileURL() throws -> URL {
        // Use stored path when available; first-launch path is set by AppDelegate before setup().
        if let stored = UserDefaults.standard.string(forKey: DatabaseManager.dbPathKey), !stored.isEmpty {
        ...
        // Fallback (should only happen if UserDefaults were cleared manually).
        return try DatabaseManager.defaultDatabaseURL()
    }
    ...
    private func runMigrations(on pool: DatabasePool) throws {
        ...
        migrator.registerMigration("v1_initial") { db in
            ...
            try db.create(table: "sync_pairs", ifNotExists: true) { t in
                ...
                t.column("checksumEnabled", .boolean).notNull().defaults(to: false)
